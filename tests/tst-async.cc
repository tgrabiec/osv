...
class timer_task_with_mutex : public timer_task {
private:
    mutex m;
public:
    timer_task_with_mutex(callback_t&& callback)
        : timer_task(std::move(callback), m)
    {}
};
...
BOOST_AUTO_TEST_CASE(test_async_task_can_be_reprogrammed_from_the_callback)
{
    ...
    task = new timer_task_with_mutex([&] {
        if (counter++ == 0) {
            task->reschedule(1_ms);
        } else {
            done.set_value(true);
...
BOOST_AUTO_TEST_CASE(test_a_task_which_is_set_far_in_future_does_not_block_new_task)
{
    ...
    timer_task_with_mutex far_task([&] {});
...
BOOST_AUTO_TEST_CASE(test_timers_with_same_expiration_time_fire_separately)
{
    ...
    timer_task_with_mutex* tasks[n_tasks];

    for (auto& task_ptr : tasks) {
        task_ptr = new timer_task_with_mutex([&] {
            if (++counter == n_tasks) {
                done.set_value(true);
            }
        });
